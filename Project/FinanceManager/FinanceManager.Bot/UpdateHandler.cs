...
public class UpdateHandler(ITelegramBotClient bot, ILogger logger) : IUpdateHandler
{
    ...
    {
        logger.Information("Received inline keyboard callback from: {CallbackQueryId}", callbackQuery.Id);
    ...
    {
        logger.Information("Received inline query from: {InlineQueryFromId}", inlineQuery.From.Id);
    ...
    {
        logger.Information("Received inline result: {ChosenInlineResultId}", chosenInlineResult.ResultId);
    ...
    {
        logger.Information("Received Poll info: {Question}", poll.Question);
